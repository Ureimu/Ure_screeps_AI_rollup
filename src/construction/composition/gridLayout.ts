...
export function ifEnoughSpace(
    ...
    while (ifNotEnough) {
        ...
        global.log(Array.from(roadExpand.keys()));
        ...
        global.log(Array.from(buildingExpand.keys()));
        ExpandList.forEach(pos => {
        ...
        for (const roadExpandPosStr of roadExpand) {
            ...
            // global.log(roadExpandPosCoord.x, roadExpandPosCoord.y);
            // global.log(terrainData[roadExpandPosCoord.y]?.[roadExpandPosCoord.x]?.[0]);
            if (
                ...
                // global.log(`road位置${roadExpandPosStr}不满足要求：terrain === "plain"，去除`);
        ...
        for (const buildingExpandPosStr of buildingExpand) {
            ...
            global.log(buildingExpandPosCoord.x, buildingExpandPosCoord.y);
            global.log(terrainData[buildingExpandPosCoord.y]?.[buildingExpandPosCoord.x]?.[0]);
            if (
                ((terrainData[buildingExpandPosCoord.y]?.[buildingExpandPosCoord.x]?.[0] as unknown) as string) ===
                "wall"
            ) {
                global.log(`building位置${buildingExpandPosStr}不满足要求：terrain !== "wall"，去除`);
    ...
    global.log(JSON.stringify(terrainData));
    global.log("已经在此位置寻找到合适布局。");
    cpu = Game.cpu.getUsed() - cpu;
    global.log(cpu);
