...
export function setChangeFindClostestByPath(bool) {
...
export function setPathClearDelay(number) {
...
export function setHostileCostMatrixClearDelay(number) {
    if (typeof number == "number" && number > 0) {
...
export function deleteCostMatrix(roomName) {
    delete costMatrixCache[roomName];
    return OK;
}
export function deltePath(fromPos, toPos, opts) {
    //if(!(fromPos instanceof RoomPosition))
    return 'not implemented';
}
export function addAvoidRooms(roomName) {
...
export function deleteAvoidRooms(roomName) {
    let splited = reg1.exec(roomName);
    if (splited && splited.length == 5) {
...
export function deletePathInRoom(roomName) {
    let splited = reg1.exec(roomName);
    if (splited && splited.length == 5) {
...
export function addAvoidExits(fromRoomName, toRoomName) {
...
export function deleteAvoidExits(fromRoomName, toRoomName) {
    let splited1 = reg1.exec(fromRoomName);
    let splited2 = reg1.exec(toRoomName);
    if (splited1 && splited1.length == 5 && splited2 && splited2.length == 5) {
