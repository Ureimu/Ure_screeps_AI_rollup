...
const Profiler = {
    ...
    callgrind() {
        const elapsedTicks = (typeof Memory.profiler.disableTick === 'number') ? (Memory.profiler.disableTick - Memory.profiler.enabledTick - 2) : (Game.time - Memory.profiler.enabledTick + 1);
        ...
        // Profiler.checkMapItem('(InitMemory)', Memory.profiler.map['(tick)'].subs);
        // Memory.profiler.map['(tick)'].subs['(InitMemory)'].calls = elapsedTicks;
        // Memory.profiler.map['(tick)'].subs['(InitMemory)'].time = Memory.profiler.InitMemoryTotalTime;
        ...
        for (const fnName of Object.keys(Memory.profiler.map)) {
            ...
            for (const callName of Object.keys(fn.subs)) {
                ...
                callsBody += `cfn=${callName}\ncalls=${call.calls} 1\n1 ${ns} ${Math.round(ns / call.calls)} ${Math.round(call.calls / elapsedTicks)}\n`;
                callsTime += call.time;
            }
            body += `\nfn=${fnName}\n1 ${Math.round((fn.time - callsTime) * 1000000)} ${Math.round((fn.time - callsTime) * 1000000) / Math.round(callsTime)} ${Math.round(Math.round(callsTime) / elapsedTicks)}\n${callsBody}`;
        }
        return body;
    },
...
function getInitMemoryUsedProfileFunction(fn, functionName) {
    const fnName = functionName || fn.name;
    if (!fnName) {
        console.log('Couldn\'t find a function name for - ', fn);
        console.log('Will not profile this function.');
...
function wrapGetInitMemoryUsedFunction(name, originalFunction) {
    if (originalFunction.profilerWrapped) { throw new AlreadyWrappedError(); }
    function wrappedFunction() {
